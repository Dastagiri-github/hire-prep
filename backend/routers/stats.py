...
def get_user_stats(
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to view these stats"
        )
    ...
    submissions = (
        ...
        .filter(
            models.Submission.user_id == user_id, models.Submission.status == "Accepted"
        )
        .all()
    )
    ...
    topic_data = [
        {"subject": tag, "A": count, "fullMark": 10}
    ...
    all_submissions = (
        db.query(models.Submission).filter(models.Submission.user_id == user_id).all()
    )
