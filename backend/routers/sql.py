...
def create_chapter(
    chapter: schemas.SQLChapterCreate, db: Session = Depends(database.get_db)
):
...
def get_chapter(chapter_id: int, db: Session = Depends(database.get_db)):
    chapter = (
        db.query(models.SQLChapter).filter(models.SQLChapter.id == chapter_id).first()
    )
    if not chapter:
...
def create_problem(
    problem: schemas.SQLProblemCreate, db: Session = Depends(database.get_db)
):
...
def get_problem(problem_id: int, db: Session = Depends(database.get_db)):
    problem = (
        db.query(models.SQLProblem).filter(models.SQLProblem.id == problem_id).first()
    )
    if not problem:
...
def run_sql(
    ...
    problem = (
        db.query(models.SQLProblem)
        .filter(models.SQLProblem.id == request.problem_id)
        .first()
    )
    if not problem:
        raise HTTPException(status_code=404, detail="Problem not found")
    ...
    result = execute_sql_problem(
        problem.setup_sql, problem.solution_sql, request.user_query
    )
