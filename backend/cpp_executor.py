...
async def execute_cpp_code(code: str, input_data: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        exe_file_path = os.path.join(temp_dir, "solution.exe" if os.name == 'nt' else "solution")
        ...
        if not gpp_path:
            return {"output": "", "error": "g++ compiler not found. Please install MinGW or G++."}
...
def parse_input_to_stdin_with_sizes(input_str: str) -> str:
    ...
    def replace_array(match):
        ...
        elements = [e.strip().strip("'").strip('"') for e in content.split(',') if e.strip()]
        size = len(elements)
