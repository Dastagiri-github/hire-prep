...
export default function SQLWorkspace({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="flex flex-col h-screen pt-16 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-[#0a0f1e] to-black">
            {/* Header */}
            <div className="h-16 border-b border-white/5 bg-gray-900/50 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-40">
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-2 mr-2 border-r border-white/10 pr-6">
                        <Link
                            ...
                            className={`p-2 rounded-lg transition-all ${prevProblem ? 'hover:bg-white/10 text-gray-400 hover:text-white' : 'text-gray-800 cursor-not-allowed pointer-events-none'}`}
                        ...
                        <Link
                            ...
                            className={`p-2 rounded-lg transition-all ${nextProblem ? 'hover:bg-white/10 text-gray-400 hover:text-white' : 'text-gray-800 cursor-not-allowed pointer-events-none'}`}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${getDifficultyColor(problem.difficulty)}`}>
                                    {problem.difficulty}
                                </span>
                            </div>
                        </div>
                ...
                <button
                    onClick={handleRun}
                    disabled={loading}
                    className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white rounded-xl font-bold transition-all duration-300 shadow-lg shadow-green-500/20 hover:shadow-green-500/40 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            <div className="flex-1 flex overflow-hidden">
                {/* Sidebar */}
                <div className={`border-r border-white/5 bg-gray-900/30 flex flex-col overflow-y-auto shrink-0 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-72 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
                    <div className="p-6">
                                    ...
                                    <div className="space-y-1">
                                        {chapter.problems.map(p => (
                                            <Link
                                                key={p.id}
                                                href={`/sql/${p.id}`}
                                                className={`block px-4 py-2 text-sm rounded-lg transition-all duration-200 ${p.id === problem.id
                ...
                <div className="w-1/3 border-r border-white/5 bg-gray-900/20 flex flex-col min-w-[350px] backdrop-blur-sm">
                    <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                        <div className="prose prose-invert max-w-none">
                            <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                Problem Description
                            </h2>
                            <div className="text-gray-300 leading-relaxed bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-[#1e1e1e]">
                    ...
                    {/* Results */}
                    <div className="h-[45%] bg-gray-900 border-t border-white/10 flex flex-col shadow-[0_-10px_40px_rgba(0,0,0,0.3)] z-10">
